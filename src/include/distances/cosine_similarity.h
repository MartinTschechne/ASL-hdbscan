...
/**
 * @brief SIMD-vectorized (with FMAs) version of function CosineSimilarity
 *
 * @param a Vector of length >= n
 * @param b Vector of length >= n
 * @param n Size of the vectors
 * @return double
 */
inline double CosineSimilarity_Vectorized_FMA(
    const double* a, const double* b, size_t n) {
    __m256d a_val, b_val;
    __m256d mag_a_accum = _mm256_setzero_pd(),
        mag_b_accum = _mm256_setzero_pd(),
    ...
    for (; i < m - 3; i += 4) {
        a_val = _mm256_loadu_pd(&a[i]);
        b_val = _mm256_loadu_pd(&b[i]);
        dot_product_accum = _mm256_fmadd_pd(a_val, b_val, dot_product_accum);
        mag_a_accum = _mm256_fmadd_pd(a_val, a_val, mag_a_accum);
        mag_b_accum = _mm256_fmadd_pd(b_val, b_val, mag_b_accum);
    }
...
#endif //__AVX2__
#endif //TEAM33_DISTANCES_COSINE_SIMILARITY_H
